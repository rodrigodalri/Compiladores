...
prog : listadeclaracoes { root = $$; astreePrint(0, root);}
     ;
...
cmd : TK_IDENTIFIER '=' exp			{ $$ = astreeCreate(ASTREE_ATR, $1, $3, 0, 0, 0); }
    | TK_IDENTIFIER '[' exp ']' '=' exp		{ $$ = astreeCreate(ASTREE_AATR,$1, $3, $6, 0, 0); }				
...
exp : TK_IDENTIFIER 				{ $$ = astreeCreate(ASTREE_SYMBOL, $1, 0, 0, 0, 0); }
    | TK_IDENTIFIER '[' exp ']' 		{ $$ = astreeCreate(ASTREE_AACC, $1, $3, 0, 0, 0); }
...
int yyerror(char *err){
	...
	fprintf(stderr, "Problema Parser! ERRO linha = %d\n", getLineNumber());
	exit(3);
}
