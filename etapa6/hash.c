...
void asmAddImm(){
	...
	{	
		Hash_Node* aux;
		for(aux = Table[i]; aux; aux = aux->next){
			if(aux->type == SYMBOL_LIT_INT && aux->datatype <= SYMBOL_DATATYPE_BYTE){ //colocar outros symbols lits na condição
				    fprintf(fout, "\t.globl	_%s\n"
				    "\t.data\n"
				    "\t.type	_%s, @object\n"
				    "\t.size	_%s, 4\n"
				    "_%s:\n" ,aux->text, aux->text, aux->text, aux->text);
                if(aux->datatype == SYMBOL_DATATYPE_FLOAT){
					
				    fprintf(fout, "\t.float	%s\n", aux->text);
			    }
				else if(aux->datatype == SYMBOL_DATATYPE_BYTE){
				    fprintf(fout, "\t.long	%s\n", aux->text);
			    }
			}
		}
